import { CheckList, ProgressBar, SectionHeading } from "../ui";

export function StepDeployments({
  formData,
  toggleArrayItem,
  schedules,
  trainingTypes,
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          ...
          <CheckList
            items={coaches}
            selectedIds={formData.coachIds}
            onToggle={(id) => toggleArrayItem("coachIds", id)}
            renderLabel={(c) => c.name || `Coach #${c.id}`}
          />
        </div>
      </div>
